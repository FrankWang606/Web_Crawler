...
CURL *easy_handle_init(CURLM *cm, RECV_BUF *ptr, const char *url);
...
{       
    //CURL *curl_handle[100];
    CURL *eh=NULL;
    CURLM *cm=NULL;
    CURLcode res;
    RECV_BUF recv_buf[100];
    CURLMsg *msg=NULL;
    int req_now;   
    int still_running=0;
    int msgs_left=0;
    ...
    e.data = (void*) 1; 
        ...
        if(show_view==1){
            op_work = fopen(view_file,"w");
        }
        opng = fopen("png_urls.txt","w");
        //fclose(op2);
        hcreate(5000);
        ...
        while(1){
            active_connect=0;
            still_running=0;
            for(int i=0;i<t_connect;i++){
                while(1){
                    if(todo_current>= todo_size){
                        break;
                    }
                    e.key = todo_url[todo_current];
                    if(hsearch(e,FIND)){
                        todo_current++;
                        continue;
                    }else{
                ...
                easy_handle_init(cm,&recv_buf[active_connect], todo_url[todo_current]);
                active_connect++;
                todo_current++;   
            }
            ...
            //printf("%d  %d\n",png_done,todo_size);
            if(todo_current>=todo_size || png_done==1){
                break;
            }
        }
        fclose(opng);
